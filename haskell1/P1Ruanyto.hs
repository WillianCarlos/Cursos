...
{-
a) recebe uma funcão qualquer aplica a si mesma e a internerna x
...
{-
Um Aluno possui trˆes value constructors SI, ADS e Ambos com campos. Os campos para estes value constructors
s˜ao nome e nota (Sum Double). Um Aluno tamb´em possui um value constructor Fora (indica evas˜ao ou t´ermino)
sem campo algum. Para obter a soma das notas dos alunos, ´e necess´ario combinar (para somar as notas)
apenas alunos de SI ou apenas de ADS. Caso dois alunos dos diferentes cursos sejam combinados, eles caem no
value constructor Ambos (h´a alunos em dp de SI fazendo esta disciplina com a turma de ADS). Ou seja, dois
value constructors iguais sempre se combinam igualmente. Caso os value constructors sejam diferentes eles se
combinar˜ao em Ambos, sempre. Dada esta situa¸c˜ao:
(a) (0.25) Crie o tipo Aluno;
(b) (1.25) Crie uma instˆancia de Monoid para Aluno respeitando o problema. Os nomes sempre se combinar˜ao
nas mensagens ”Total SI: ”, ”Total ADS: ” ou ”Total Ambos”.
(c) (1.0) Fa¸ca uma fun¸c˜ao somarNotas que recebe um [Aluno] e retorna um Aluno informando a soma de
todas notas.
-}

data Aluno = SI {nome::String, nota::Sum Double}
            |ADS{nome::String, nota::Sum Double}
            |Ambos{nome::String, nota::Sum Double}
            |Fora deriving Show


instance Monoid Aluno where
    mempty = Fora
    mappend (SI x y) (SI a b) = SI (x <> a) (y <> b)
    mappend (ADS x y) (ADS a b) = ADS (x <> a) (y <> b)
    mappend (SI x y) (ADS a b) = Ambos (x <> a) (y <> b)
    mappend (ADS x y) (SI a b) = Ambos (x <> a) (y <> b)
    mappend (Ambos x y) (SI a b) = Ambos (x <> a) (y <> b)
    mappend (SI x y) (Ambos a b) = Ambos (x <> a) (y <> b)
    mappend (Ambos x y) (ADS a b) = Ambos (x <> a) (y <> b)
    mappend (ADS x y) (Ambos a b) = Ambos (x <> a) (y <> b)